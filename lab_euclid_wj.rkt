...
;; _______________________________________________Problem 3_______________________________________________ 
...
(define (waterjug jugA jugB goal)             ;; Main Function
  (wj-help 0 0 jugA jugB goal))               ;; Don't have to edit, already given.


(define (wj-help fillA fillB maxA maxB goal)
  (cond
    [ (equal? fillB goal)         (list (list fillA fillB)) ]                                                                       ;; case 1. base case
    [ (equal? fillB maxB)         (cons (list fillA fillB) (wj-help fillA 0 maxA maxB goal)) ]                                      ;; case 2. empty B procedure
    [ (zero? fillA)               (cons (list fillA fillB) (wj-help maxA fillB maxA maxB goal)) ]                                   ;; case 3. fill A procedure
    [ (< maxB (+ fillA fillB))    (cons (list fillA fillB) (wj-help (- fillA (- maxB fillB)) maxB maxA maxB goal)) ]                ;; case 4. pour A into B 
    [ else                        (cons (list fillA fillB) (wj-help 0 (+ fillA fillB) maxA maxB goal)) ]                            ;; case 5. pour A into B
  )
)
